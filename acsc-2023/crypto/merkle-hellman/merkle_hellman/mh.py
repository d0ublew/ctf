...
def encrypt(pt, public_key):
    return str(sum([(int(bin(int(binascii.hexlify(pt.encode()), 16))[2:].rjust(
        len(pt) * 8, "0")[i]) * public_key[i]) for i in range(0, len(public_key))]))
...
def GramSchmidt(M):
    try:
        ...
        for j in range(1, len(M)):
            orthG.append(M[j])
            for i in range(0, j):
                projection_coefficients[str(
                    i) + str(j)] = (dot_product(orthG[i], M[j])) / (dot_product(orthG[i], orthG[i]))
                orthG[j] = vsum(
                    orthG[j], scalar_product(-1 * projection_coefficients[str(i) + str(j)], orthG[i]))
            ...
            for j in range(1, len(M)):
                for i in range(j - 1, -1, -1):
                    if abs(GSpc[str(i) + str(j)]) > 1 / 2:
                        M[j] = vsum(M[j], scalar_product(-1 *
                                    round(GSpc[str(i) + str(j)]), M[i]))
            GSoG, GSpc = GramSchmidt(M)
            try:
                for j in range(0, len(M) - 1):
                    tmp0 = vsum(
                        GSoG[j + 1], scalar_product(GSpc[str(j) + str(j + 1)], GSoG[j]))
                    if dot_product(tmp0, tmp0) < d * \
                            (dot_product(GSoG[j], GSoG[j])):
                        tmp1 = M[j]
                        M[j] = M[j + 1]
                        M[j + 1] = tmp1
                        raise Exception()
...
def break_cipher(ct, public_key):
    try:
        ...
        M = [[1 if i == j else 0 for i in range(
            0, len(public_key))] for j in range(0, len(public_key))]
        ...
        for vector in short_vectors:
            try:
                ...
                for n in vector:
                    ...
                    if n != 1 and n != 0:
                        raise Exception()
                print("\nPossible plaitext found > " + \
                      binascii.unhexlify(hex(int(cur[:-1], 2))[2:].encode()).decode() + "\n")
                flag = 1
            except BaseException:
                continue
...
# The Main Function handles user input, menu conditions and the retrieval
# of information from provided text files
    ...
    while True:
        ...
        if op == "1":
            ...
            if op1 == "1":
                try:
                    pub_file = str(
                        input("Enter the name of your public key file(file should have one number per line)\n> "))
                    public_key = []
        ...
        # Main menu option 2
            ...
            # Decrypt menu option 2
                ...
                try:
                    pub_file = str(
                        input("Enter the name of a public key file\n> "))
                    public_key = []
