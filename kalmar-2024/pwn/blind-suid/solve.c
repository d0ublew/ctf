...
int main() {
    ...
    if (child_pid == 0) {
        ...
        u64 rip = ptrace(PTRACE_PEEKUSER, child_pid,
        ...
        u64 rsp = ptrace(PTRACE_PEEKUSER, child_pid,
                         offsetof(struct user, regs.rsp), NULL);

        for (; read_memory(child_pid, rsp) != 0x3; rsp += 8)
            ; // AT_PHDR
        ...
        if (kill(child_pid, 9)) perror("kill");
