...
void __attribute__((constructor)) setup() {
    ...
    for (bin *qb = quick_bins; qb < quick_bins + QUICK_BIN_MAX / UNIT_SIZE - 1;
         ++qb) {
        qb->nmemb = 0;
        qb->head = NULL;
        qb->rearrange = false;
...
chunk_metadata *split_chunk(chunk_metadata *metadata, size_t size) {
    ...
    chunk_metadata *down_meta =
        (chunk_metadata *)((char *)metadata + metadata->size);
...
char *unlink_(chunk_metadata *curr, chunk_metadata *prev, size_t size,
              bin *free_list) {
...
char *not_malloc(size_t size) {
    ...
    if (!chunk) {
        ...
        if (size > metadata->size - UNIT_SIZE) {
            ...
            extend_mapping((void *)((size_t)top_chunk & ~(size_t)0xfff),
                           new_size);
            extend_mapping((void *)((size_t)metadata & ~(size_t)0xfff),
                           new_size);
        ...
        chunk_metadata *top_metadata =
            (chunk_metadata *)((char *)metadata + size);
