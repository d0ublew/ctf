...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f101mX = new float[min];
            this.f102mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f101mX[i2] = fArr[0];
                this.f102mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f101mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f102mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f101mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f102mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f101mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            int i6 = min - 1;
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f101mX[0] + "," + this.f102mY[0] + " end:" + this.f101mX[i6] + "," + this.f102mY[i6]);
        }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f101mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f101mX[i2]) {
