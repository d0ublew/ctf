...
FILE *get_user_save_fp(char username[USERPASS_LEN], const char *mode) {
    char fname[USERPASS_LEN + 6]; // accounted for NULL byte
...
void register_user() {
    ...
    fread(tmpbuf, 1, tl.len, stdin); // ok
    tmpbuf[tl.len] = '\0';
...
FILE *handle_auth(char username[USERPASS_LEN]) {
    ...
    if (fp == 0) {
        ...
        if (tl.tag != TAG_STR_PASSWORD || tl.len >= USERPASS_LEN) {
            print_tlv(TAG_RES_ERROR,
                      "corrupted user file, got invalid taglen %d %lld", tl.tag,
                      tl.len);
            return 0;
        }
        fread(tmpbuf2, 1, tl.len, fp); // ok
...
void view_mail() {
    ...
    // Overwrites the content of `tmpbuf`
    memcpy(tmpbuf, "from: ", 6);
    fread(tmpbuf + 6, 1, t1, fp);
    tmpbuf[6 + t1] = '\n';
    // tmpbuf += "from: " + sender
    // possible to have garbage username value if len(sender) < len(username)
...
void send_mail() {
    ...
    // Now `tmpbuf2` has `recipient` data
    tmpbuf2[tl.len] = '\0';
    ...
    fread(tmpbuf, 1, tl.len, stdin);
    // Now `tmpbuf` is ovewritten with message data

    pack_taglen(TAG_STR_MESSAGE, tl.len, tmpbuf2);
    // writes MESSAGE tag to recipient file
    fwrite(tmpbuf2, 1, 12, fp); // !!!
    // writes message data to recipient file
    fwrite(tmpbuf, 1, tl.len, fp);
    fflush(fp);
